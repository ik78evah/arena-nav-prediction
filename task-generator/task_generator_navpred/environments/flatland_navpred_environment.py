...
@EnvironmentFactory.register("flatland_navpred")
class FlatlandNavPredEnvironment(BaseEnvironment):
    """
    ...
    def remove_all_obstacles(self):
        for obs in range(self._obstacles_amount):
            obs_name = FlatlandNavPredEnvironment.create_obs_name(obs)
    ...
    def spawn_obstacle(self, position, yaml_path=""):
        name = FlatlandNavPredEnvironment.create_obs_name(self._obstacles_amount)

        self._spawn_model(yaml_path, name, self._namespace, position)

        self._obstacles_amount += 1
        
    def init_random_dynamic_obstacle(self, obstacle_name, **args):
        self._init_random_obstacle(obstacle_name, **args, is_dynamic=True)

    def init_random_static_obstacle(self, obstacle_name, **args):
        self._init_random_obstacle(obstacle_name, **args, is_dynamic=False)

    def _init_random_obstacle(
            self, obstacle_name, is_dynamic=False, position=[0, 0, 0], **args
        ):
        model = self._generate_random_obstacle(is_dynamic=is_dynamic, **args)

        obstacle_name_generic = FlatlandNavPredEnvironment.create_obs_name(
            self._obstacles_amount
        )
        
        param_path = "/obstacles/"
        if is_dynamic:
            param_path = param_path + "dynamic"
        else:
            param_path = param_path + "static"
                    
        rospy.set_param(f"{param_path}/{obstacle_name}/position", str(position))
        
        if len(model["plugins"]) != 0:
            rospy.set_param(f"{param_path}/{obstacle_name}/linear_vel", model["plugins"][0]["linear_velocity"])
            rospy.set_param(f"{param_path}/{obstacle_name}/angular_vel_max", model["plugins"][0]["angular_velocity_max"])
            
        obs_form = model["bodies"][0]["footprints"][0]["type"]
        rospy.set_param(f"{param_path}/{obstacle_name}/form", obs_form)
        
        
        if obs_form == "circle":
            rospy.set_param(f"{param_path}/{obstacle_name}/radius", model["bodies"][0]["footprints"][0]["radius"])
        else:
            rospy.set_param(f"{param_path}/{obstacle_name}/points", str(model["bodies"][0]["footprints"][0]["points"]))
        
        if is_dynamic:
            obstacle_name = "dynamic_" + obstacle_name
        else:
            obstacle_name = "static_" + obstacle_name
        
        
        self._spawn_model(
            yaml.dump(model), 
            obstacle_name_generic, 
            self._namespace, 
            position, 
    ...
    def respawn_obstacle(self, obstacle_name, is_dynamic=False, position=[0, 0, 0], **args):
        model = self._generate_old_obstacle(obstacle_name, is_dynamic=is_dynamic, **args)
    ...
    def _generate_old_obstacle(self, 
            obstacle_name,
            is_dynamic=False
            ):
        ...
        footprint = {
            ...
            **self._generate_old_footprint_type(is_dynamic, obstacle_name)
        }
        ...
        if is_dynamic:
            linear_vel = rospy.get_param(f"obstacles/dynamic/{obstacle_name}/linear_vel")
            angular_vel_max = rospy.get_param(f"obstacles/dynamic/{obstacle_name}/angular_vel_max")
    ...
    def _generate_random_obstacle(self, is_dynamic=False):
        """
        ...
        min_radius = rospy.get_param("/obstacles/min_radius")
        max_lin_vel = rospy.get_param("/obstacles/max_lin_vel")
        min_lin_vel = rospy.get_param("/obstacles/min_lin_vel")
        max_ang_vel = rospy.get_param("/obstacles/max_ang_vel")
        min_ang_vel = rospy.get_param("/obstacles/min_ang_vel")
        ...
        if is_dynamic:
            model['plugins'].append({
                **FlatlandRandomModel.RANDOM_MOVE_PLUGIN,
                'linear_velocity': random.uniform(min_lin_vel, max_lin_vel),
                'angular_velocity_max': max_ang_vel
            })

        return model
    
    def _generate_old_footprint_type(self, is_dynamic, obstacle_name):
        obs_type = None
        ...
        form = rospy.get_param(f"/obstacles/{obs_type}/{obstacle_name}/form")

        if form == "circle":
            radius = rospy.get_param(f"/obstacles/{obs_type}/{obstacle_name}/radius")
        ...
        points_str = rospy.get_param(f"/obstacles/{obs_type}/{obstacle_name}/points")
        
        # Converts string stored in parameters to list of list of floats
        points_str = points_str.split("], [")
        points_str = [s.replace("[[", "") for s in points_str]
        points_str = [s.replace("]]", "") for s in points_str]
        
        points = []
        
        for point in points_str:
            point_str_list = point.split(",")
            points.append([float(point_str_list[0]), float(point_str_list[1])])
    ...
    def _update_plugin_topics(self, file_content, namespace):
        ...
        for plugin in plugins:
            if FlatlandNavPredEnvironment.PLUGIN_PROPS_TO_EXTEND.get(plugin["type"]):
                prop_names = FlatlandNavPredEnvironment.PLUGIN_PROPS_TO_EXTEND.get(plugin["type"])
